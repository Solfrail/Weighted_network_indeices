...
/*
Barrat's weighted cluster(Zhang_wc.c)
-fv (int): initial vertex
-detail filename: output c_i
-al filename: output weighted average path length to file 

Calculate B.Zhang's cluster coefficient 
*/
...
struct Strength{double weight; double inv_weight; double using_weight;};
...
double List_Search_cls(struct cell *L, int *B, int u, int v);
...
void read_data(struct cell *L, int *bucket, int *degree, struct Strength *strength, int comment, int *N);
...
int main(int argc, char *argv[]){
    ...
    static int address1, address2;
    double cluster=0;

    double w_ij, w_ik, w_jk; //use for calculating weighted cluster
    double WC_numerator, WC_denominator; //重み付きクラスタ係数C_{w, i}の分子，分母
    int *degree;
    int *degree_dist;
    ...
    if((strength = (struct Strength *)malloc(sizeof(struct Strength) * N))==NULL){
      fprintf(stderr, "degree_dist: Cannot allocate memory.\n");
      return EXIT_FAILURE;
    }
    ...
    for(i=0;i<N;i++){
      ...
      non_connect[i] = NIL;
      strength[i].weight = 0;
      strength[i].inv_weight = 0;
      strength[i].using_weight = 0;
    }

    fprintf(stderr, "initialize_complete\n");
    read_data(L, bucket, degree, strength, comment, &N);

    //weight == 1 -> 重みは距離(デフォルト), weight == 2 -> 重みは距離の逆数
    if(weight == 1){
      for(i = 0; i < 2 * edge; ++i){
	      L[i].using_weight = L[i].weight;
      }
      for(i = 0; i < N; ++i){
	      strength[i].using_weight = strength[i].weight;
      }
    }else if(weight == 2){
      for(i = 0; i < 2 * edge; ++i){
      ...
      for(i = 0; i < N; ++i){
	      strength[i].using_weight = strength[i].inv_weight;
      }
    }
    ...
    //extract the hugest network in the input edgelist
    k = 0;//count non_connect_num
    first_lap = 1;
    for(j=0;j<1;j++){
      if(j%10==0){fprintf(stderr, "%d\n", j);}
    ...
    //calculate Barrat's weighted cluster coefficient
    cluster = 0;
    for(i=0;i<N;i++){
      if(i%500==0){fprintf(stderr, "%d\n",i);}
      while(non_connect_search(non_connect, i)==1){
	      i++;
      	if(i%500==0){fprintf(stderr, "%d\n",i);}
      }
      ...
      while(address1 != NIL && degree[i] > 1){
	      w_ij = L[address1].using_weight;
	      address2 = L[address1].next;
          while(address2 != NIL){
            w_ik = L[address2].using_weight;
            //計算量を減らすための枝切り
            if(degree[L[address1].vertex] < degree[L[address2].vertex]){
              w_jk = List_Search_cls(L, bucket, L[address1].vertex, L[address2].vertex);
            }
            else{w_jk = List_Search_cls(L, bucket, L[address2].vertex, L[address1].vertex);}
            if(w_jk != 0.0){WC_numerator += w_ij + w_ik;}
            address2 = L[address2].next;
          }
	      address1 = L[address1].next;
      }
      WC_denominator += strength[i].using_weight * (degree[i] - 1);
      if(degree[i] > 1)cluster += WC_numerator / WC_denominator;
      if(detail!=0){
	    fp = fopen(argv[detail], "a");
      if(degree[i] > 1)fprintf(fp, "%d %lf\n", i, WC_numerator/WC_denominator);
      if(degree[i] <= 1)fprintf(fp, "%d 0\n", i);
      fclose(fp);
      }
    }
    cluster /= (double)connect_num;
    fprintf(stderr, "#fixed_cluster = %lf\n", cluster);
    fprintf(stdout, "%lf\n", cluster);
...
void read_data(struct cell *L, int *bucket, int *degree, struct Strength *strength, int comment, int *N){
  ...
  while(fgets(buf, sizeof(buf), stdin)!=NULL){
    if(com == 0){
      ...
      tok = strtok(NULL, space);
      weight = atof(tok);
      strength[u].weight += weight;
      strength[v].weight += weight;
      tok = strtok(NULL, space);
      inv_weight = atof(tok);
      strength[u].inv_weight += inv_weight;
      strength[v].inv_weight += inv_weight;
      inv_weight = atof(tok);
