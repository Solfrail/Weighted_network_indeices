...
/*
Onnela's weighted cluster(Zhang_wc.c)
...
int main(int argc, char *argv[]){
    ...
    first_lap = 1;
    for(j=0;j<1;j++){ //Calculate Only cluster
    ...
    //calculate Onnela's weighted cluster coefficient
    cluster = 0;
    for(i=0;i<N;i++){
	...
	if(i%500==0){fprintf(stderr, "%d\n",i);}
      ...
      while(address1 != NIL && degree[i] > 1){
        ...
        while(address2 != NIL){
          ...
          WC_numerator += pow(w_ij * w_ik * w_jk, 1.0/3.0);
          address2 = L[address2].next;
        }
        
        address1 = L[address1].next;
      }
      WC_denominator += degree[i] * (degree[i] - 1) / 2;
